...
def salidaToSpeech(entrada):
    ...
    for i in range(0,len(entrada)):
        ...
        if not len(buscarNoTexToEs):
            auxSalidaCMATH=latexToCMathml(entrada[i])
            auxSalidaSpeech=CMATHtoEs(auxSalidaCMATH)
            print("/////////")
        else:
            print("?????????????")
            auxSalidaCMATH=entrada[i].replace("$$","")
        ...
        salidaCMATH.append(auxSalidaCMATH)
    return salida, salidaCMATH
...
def echoToSpeech(entrada,entradaNoEcho):
    ...
    for i in range(0,len(entrada)):
        #buscarNoTexToEs=re
        auxSalidaSpeech=latexToCMathml(entrada[i])
        ###print("-------------------------------")
        ###print(auxSalidaSpeech)
        auxSalidaSpeech=re.sub(r"->.*\n","",auxSalidaSpeech)
...
def procesar():
    if request.method == 'POST':
        ...
        ###print(listaRespuestas)
        salidaSpeech,salidaCMATH=salidaToSpeech(listaRespuestas)
        print("7--- %s seconds ---" % (time.time() - start_time))
        ##print(xmltree)
        print("++++++++++++++")
        print(salidaCMATH)
        respuestaJson={"entradaScript":entradaNoEcho,"salida":listaRespuestas,"entradaSpeech":echoSpeech,"salidaSpeech":salidaSpeech, "salidaCMATH":salidaCMATH}
